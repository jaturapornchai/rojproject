...
import {
    ...
    type ReportFilters,
    type EmailSchedule
} from '@/lib/reports/srr20016';
import { useReportFilters, useMasterData } from '@/hooks/reports/srr20016';
import { FilterPanel, FilterSummary } from '@/components/reports/srr20016';
...
export default function ScheduleManagement() {
    ...
    // ใช้ shared hooks
    const { products, productGroups, productBrands } = useMasterData();
    const {
        filters,
    ...
    const getFilterSummary = (filterConfig: string | ReportFilters | undefined) => {
        ...
        if (filters.product.filterType !== 'all') {
            if (filters.product.filterType === 'single' && filters.product.selectedProduct) {
                ...
                items.push(`สินค้า: ${product?.name_1 || filters.product.selectedProduct}`);
            } else if (filters.product.filterType === 'range' && filters.product.rangeStart && filters.product.rangeEnd) {
                items.push(`สินค้า: ${filters.product.rangeStart} - ${filters.product.rangeEnd}`);
            } else if (filters.product.filterType === 'multiple' && filters.product.selectedProducts.length > 0) {
                items.push(`สินค้า: ${filters.product.selectedProducts.length} รายการ`);
            }
        }
        ...
        if (filters.productGroup.filterType !== 'all') {
            if (filters.productGroup.filterType === 'single' && filters.productGroup.selectedProductGroup) {
                ...
                items.push(`กลุ่มสินค้า: ${group?.name_1 || filters.productGroup.selectedProductGroup}`);
            } else if (filters.productGroup.filterType === 'range' && filters.productGroup.rangeStart && filters.productGroup.rangeEnd) {
                items.push(`กลุ่มสินค้า: ${filters.productGroup.rangeStart} - ${filters.productGroup.rangeEnd}`);
            } else if (filters.productGroup.filterType === 'multiple' && filters.productGroup.selectedProductGroups.length > 0) {
                items.push(`กลุ่มสินค้า: ${filters.productGroup.selectedProductGroups.length} รายการ`);
            }
        }
        ...
        if (filters.productBrand.filterType !== 'all') {
            if (filters.productBrand.filterType === 'single' && filters.productBrand.selectedProductBrand) {
                ...
                items.push(`ยี่ห้อ: ${brand?.name_1 || filters.productBrand.selectedProductBrand}`);
            } else if (filters.productBrand.filterType === 'range' && filters.productBrand.rangeStart && filters.productBrand.rangeEnd) {
                items.push(`ยี่ห้อ: ${filters.productBrand.rangeStart} - ${filters.productBrand.rangeEnd}`);
            } else if (filters.productBrand.filterType === 'multiple' && filters.productBrand.selectedProductBrands.length > 0) {
                items.push(`ยี่ห้อ: ${filters.productBrand.selectedProductBrands.length} รายการ`);
            }
        }
                    ...
                    <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 space-y-4 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <button
                                ...
                                className="px-4 py-2 text-white bg-rose-600 hover:bg-rose-700 rounded-lg font-medium transition-colors shadow-sm shadow-rose-200"
                            >
                                ยืนยันส่งทันที
                            </button>
                        </div>
            ...
            <div className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/reports/srr20016" className="text-slate-400 hover:text-rose-600 transition-colors">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-rose-600 text-white rounded-lg hover:bg-rose-700 transition font-medium text-sm shadow-sm"
                        >
                            + เพิ่มตารางส่ง
                        </button>
                    </div>
            ...
            {showJson && (
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="bg-slate-900 rounded-xl shadow-lg p-6 overflow-hidden">
                        ...
                        <pre className="text-xs font-mono text-rose-400 overflow-auto max-h-96 bg-slate-950 p-4 rounded-lg">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="border-t border-slate-200 pt-6">
                                <button
                                    ...
                                    className="flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-rose-600 transition-colors"
                                >
                    ...
                    {loading ? (
                        ...
                        <div className="divide-y divide-slate-200">
                            {schedules.map((schedule) => {
                                const filterSummary = getFilterSummary(schedule.filter_config);
                                return (
                                        ...
                                        <div className="flex items-start justify-between">
                                                ...
                                                {filterSummary && (
                                                    <div className="mt-2 flex flex-wrap gap-2">
                                                        {filterSummary.map((item, index) => (
                                                            <span key={index} className="px-2 py-1 bg-rose-100 text-rose-800 text-xs rounded-full">
                                                                {item}
                                                            </span>
                                                        ))}
                                                    </div>
                                            ...
                                            <div className="flex gap-2 ml-4">
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 text-sm bg-rose-600 text-white rounded-lg hover:bg-rose-700 transition disabled:bg-slate-300 disabled:cursor-not-allowed"
                                                >
