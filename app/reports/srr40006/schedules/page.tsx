...
import {
    ...
    type ReportFilters,
    type EmailSchedule
} from '@/lib/reports/srr40006';
import { useReportFilters, useMasterData } from '@/hooks/reports/srr40006';
import { FilterPanel, FilterSummary } from '@/components/reports/srr40006';
...
export default function ScheduleManagement() {
    ...
    // ใช้ shared hooks
    const { productGroups, warehouses, brands } = useMasterData();
    const {
        filters,
    ...
    const handleEdit = (schedule: EmailSchedule) => {
        ...
        if (schedule.filter_config) {
            const parsedFilters = deserializeFilters(schedule.filter_config as unknown as string);
    ...
    const getFilterSummary = (filterConfig: ReportFilters | undefined) => {
        if (!filterConfig) return null;
        const filters = typeof filterConfig === 'string' ? deserializeFilters(filterConfig) : filterConfig;
        const items: string[] = [];
        ...
        if (filters.productGroup.filterType !== 'all') {
            if (filters.productGroup.filterType === 'single' && filters.productGroup.selectedProductGroup) {
                ...
                items.push(`กลุ่ม: ${pg?.name_1 || filters.productGroup.selectedProductGroup}`);
            } else if (filters.productGroup.filterType === 'range' && filters.productGroup.rangeStart && filters.productGroup.rangeEnd) {
                items.push(`กลุ่ม: ${filters.productGroup.rangeStart} - ${filters.productGroup.rangeEnd}`);
            } else if (filters.productGroup.filterType === 'multiple' && filters.productGroup.selectedProductGroups.length > 0) {
                items.push(`กลุ่ม: ${filters.productGroup.selectedProductGroups.length} รายการ`);
            }
        }
        ...
        if (filters.warehouse.filterType !== 'all') {
            if (filters.warehouse.filterType === 'single' && filters.warehouse.selectedWarehouse) {
                ...
                items.push(`คลัง: ${wh?.name_1 || filters.warehouse.selectedWarehouse}`);
            } else if (filters.warehouse.filterType === 'range' && filters.warehouse.rangeStart && filters.warehouse.rangeEnd) {
                items.push(`คลัง: ${filters.warehouse.rangeStart} - ${filters.warehouse.rangeEnd}`);
            } else if (filters.warehouse.filterType === 'multiple' && filters.warehouse.selectedWarehouses.length > 0) {
                items.push(`คลัง: ${filters.warehouse.selectedWarehouses.length} รายการ`);
            }
        }
        ...
        if (filters.brand.filterType !== 'all') {
            if (filters.brand.filterType === 'single' && filters.brand.selectedBrand) {
                ...
                items.push(`ยี่ห้อ: ${b?.name_1 || filters.brand.selectedBrand}`);
            } else if (filters.brand.filterType === 'range' && filters.brand.rangeStart && filters.brand.rangeEnd) {
                items.push(`ยี่ห้อ: ${filters.brand.rangeStart} - ${filters.brand.rangeEnd}`);
            } else if (filters.brand.filterType === 'multiple' && filters.brand.selectedBrands.length > 0) {
                items.push(`ยี่ห้อ: ${filters.brand.selectedBrands.length} รายการ`);
            }
        }
            ...
            <div className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/reports/srr40006" className="text-slate-400 hover:text-emerald-600 transition-colors">
                                ...
                                {showFilterPanel && (
                                    <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                                        <FilterPanel
                                            ...
                                            onBrandFilterTypeChange={setBrandFilterType}
                                            onSelectedBrandChange={setSelectedBrand}
                                            onBrandRangeStartChange={setBrandRangeStart}
                                            onBrandRangeEndChange={setBrandRangeEnd}
