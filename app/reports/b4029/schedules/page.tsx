...
import {
    ...
    type ReportFilters,
    type EmailSchedule
} from '@/lib/reports/b4029';
import { useReportFilters, useMasterData } from '@/hooks/reports/b4029';
import { FilterPanel } from '@/components/reports/b4029';
    ...
    const getFilterSummary = (filterConfig: string | undefined) => {
        ...
        if (filters.customer.filterType !== 'all') {
            if (filters.customer.filterType === 'single' && filters.customer.selectedCustomer) {
                const customer = customers.find(c => c.code === filters.customer.selectedCustomer);
                items.push(`ลูกค้า: ${customer?.name_1 || filters.customer.selectedCustomer}`);
            } else if (filters.customer.filterType === 'range' && filters.customer.rangeStart && filters.customer.rangeEnd) {
                items.push(`ลูกค้า: ${filters.customer.rangeStart} - ${filters.customer.rangeEnd}`);
            } else if (filters.customer.filterType === 'multiple' && filters.customer.selectedCustomers.length > 0) {
                items.push(`ลูกค้า: ${filters.customer.selectedCustomers.length} รายการ`);
            }
        }

        if (filters.branch.filterType !== 'all') {
            if (filters.branch.filterType === 'single' && filters.branch.selectedBranch) {
                const branch = branches.find(b => b.code === filters.branch.selectedBranch);
                items.push(`สาขา: ${branch?.name_1 || filters.branch.selectedBranch}`);
            } else if (filters.branch.filterType === 'range' && filters.branch.rangeStart && filters.branch.rangeEnd) {
                items.push(`สาขา: ${filters.branch.rangeStart} - ${filters.branch.rangeEnd}`);
            } else if (filters.branch.filterType === 'multiple' && filters.branch.selectedBranches.length > 0) {
                items.push(`สาขา: ${filters.branch.selectedBranches.length} รายการ`);
            }
        }
            ...
            <div className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/reports/b4029" className="text-slate-400 hover:text-emerald-600 transition-colors">
